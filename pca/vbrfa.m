...
% $$$   end
  ...
  if options.common_tau
    ...
    KL_tau = KL_tau - gamma_logpdf(tau(1),p.a_tau,p.b_tau,logtau(1));
  else
    KL_tau = KL_tau - sum(gamma_entropy(a_tau, b_tau));
    KL_tau = KL_tau - sum(gamma_logpdf(tau,p.a_tau,p.b_tau,logtau));
  end
  
  % KL-term: <log q(alpha)> - <log p(alpha)>
  KL_alpha = 0;
  KL_alpha = KL_alpha - sum(gamma_entropy(a_alpha, b_alpha));
  KL_alpha = KL_alpha - sum(gamma_logpdf(alpha, p.a_alpha, p.b_alpha, logalpha));
  
  % KL-term: <log q(beta)> - <log p(beta)>
  KL_beta = 0;
  KL_beta = KL_beta - sum(gamma_entropy(a_beta, b_beta));
  KL_beta = KL_beta - sum(gamma_logpdf(beta, p.a_beta, p.b_beta, logbeta));
 ...
 %     disp('Update nu');
    ...
    KL_U = KL_U - sum(gamma_logpdf(U(1,Nmv),nu(1)/2,nu(1)/2,logU(1,Nmv)));
...
% $$$     kl_w(iter) = KL_w;
  ...
  if (now-lastsave)*3600*24 >= options.autosavetime
    ...
    save(options.autosavefile, 'W','X','tau','U','nu');
