...
function Ival = metoffice_get_valset( remove_val, time, Itrain )

datapath = metoffice_get_path();

if nargin == 1
    dataset = 'hadsst2d1.mat';

    warning( sprintf( 'Load validation set for %s', dataset ) )
    
    load( [ datapath, dataset ], 'sst', 'time', 'lat', 'lon', 'clim' )
% $$$     load( [ datapath '/RecTest/DATASETS/' dataset ],...
