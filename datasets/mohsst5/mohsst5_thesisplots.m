function [data,Qgp,Qvb,ax] = mohsst5_thesisplots(data, Qgp, Qvb)
...
%  varY = diag(XX*WW) / (N*M) - mean(Yh(:))^2 / length(XX);
  ...
  figure
  % slow comps
  vars = sort(varY(1:5),'descend');
  bar(1:5,vars,'r');
  hold on
  vars = sort(varY(6:10),'descend');
  bar(6:10,vars,'g');
  vars = sort(varY(11:15),'descend');
  bar(11:15,vars,'b');
  vars = sort(varY(16:80),'descend');
  bar(16:80,vars,'w');
  ...
  xlabel('component');
  set_ticks_fontsize(gca,5);
  set_label_fontsize(gca, 7);
  set_figure_size(gcf, 12, 6);
  set_plot_style(gca);
  filename = ['/home/jluttine/thesis/figures/' ...
              'fig_experiment_explained_variance.eps'];
  print(gcf, '-depsc2', filename);
  % Plot temporal components explaining most variance
  comps = 1:4;
  hax = tsgpplot(data.time(:), X(inds(comps),:)', 2* ...
                 sqrt(varX(inds(comps),:))');
  % Publication style
  for j = 1:length(hax)
    ...
    %ylabel(hax(j), sprintf('x_{%d}', inds(comps(j))));
  end
  set_subplot_positions(hax, 4,1, [0.05 0.01 0.01 0.3], [0.01 0.01]);
  set_ticks_fontsize(hax, 5);
  set_label_fontsize(hax, 7);
  set_figure_size(gcf, 13, 3);
  filename = ['/home/jluttine/thesis/figures/' ...
              'fig_experiment_ranked_timeseries.eps'];
  print(gcf, '-depsc2', filename);
  % Plot spatial components explaining most variance
  figure
  mapproj('global-ellipse');
  [LON,LAT] = meshgrid(data.longitude, data.latitude);
  h_cb = [];
  ...
  for d=1:length(comps)
    subplot(1,length(comps),d)
    set(gca, 'xtick', [], 'ytick', []);
    mappcolor(LON, LAT, reshape(W(:,inds(comps(d))), size(LON)));
    hax(d) = gca;
  ...
  filename = ['/home/jluttine/thesis/figures/' ...
              'fig_experiment_ranked_loadings.eps'];
  print(gcf, '-depsc2', filename);
